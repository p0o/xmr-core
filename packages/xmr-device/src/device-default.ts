...
import * as crypto from "@xmr-core/xmr-crypto-ops";
import {
	secret_key_to_public_key,
	generate_keys,
	random_keypair,
} from "@xmr-core/xmr-key-utils";
import { KeyPair, Commit } from "@xmr-core/xmr-types";
import { encrypt_payment_id } from "@xmr-core/xmr-pid";
...
export class DefaultDevice implements HWDevice {
	...
	public async ecdhEncode(
		...
		return crypto.encode_ecdh(unmasked, sharedSec);
	}
	...
	public async ecdhDecode(
		...
		return crypto.decode_ecdh(masked, sharedSec);
	}
	...
	public async mlsag_hash(toHash: KeyV): Promise<Key> {
		return crypto.hash_to_scalar(toHash.join(""));
	}
	...
	public async mlsag_sign(
		...
		for (let j = 0; j < rows; j++) {
			ss[j] = crypto.sc_mulsub(c, xx[j], alpha[j]);
		}
		return ss;
	}
